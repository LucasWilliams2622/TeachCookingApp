  ...
  return (
    <View style={{ marginTop: 36, width: windowWidth - 460 }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} showsHorizontalScrollIndicator={false} style={{ marginBottom: 0 }}>
        <ScrollView horizontal={true} showsVerticalScrollIndicator={false} showsHorizontalScrollIndicator={false} style={{ marginTop: 40, marginBottom: 50 }}>
          <Table style={{}}>
            <Row
              data={tableHead}
              widthArr={widthArr}
            ...
            />
            {
              array.map((rowData, index) => {
                const isLastItem = index === array.length - 1; // Kiểm tra xem item hiện tại có phải là item cuối cùng hay không
