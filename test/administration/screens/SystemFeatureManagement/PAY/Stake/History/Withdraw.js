...
const Withdraw = () => {
  const tableHead = ["ID", "Date", "Quantity", "Withdrawer","Currency", "Ref", 'Status'];
  const widthArr = [90, 120, 120, 140, 100, 100, 90];

  let array = myData.SFM_PAY_Wallet_History.map(obj => [obj.id, obj.date, obj.quantity, obj.withdrawer, obj.currency, obj.ref, obj.status,]);
