...
const OrderComplaintHistory = () => {
  const navigation = useNavigation();
  const tableHead = ["No", "ID order", "Store’s name", "Total price", "Created date", "Request", "Reason", "Detail", "Status"];
  const widthArr = [70, 120, 140, 120, 120, 160, 160, 160, 100];

  const asset = [{ "status": true }, { "status": false }, { "status": true }, { "status": true },{ "status": false }, { "status": false }, { "status": false }, { "status": false }, { "status": false }, { "status": false }, ]
  let array = asset.map(obj => ["1", "127362", "Dì Năm", "100.000đ", "01/06/2023 09:20 am", "Trả hàng - Hoàn tiền", "Giao nhầm sản phẩm", "Tôi đặt màu xanh nhưng giao màu trắng", obj.status]);
...
export default OrderComplaintHistory
